...
int ArquivoLido(Arquivo *arq, char *NameArq){
    ...
    if(strcmp(arq->nomeArquivo,NameArq)==1) return 1;
    return ArquivoLido(arq->prox, NameArq);
}

int* realocaLinOcorr(int* ptr, int* size, int t, char palavra, int l){
    int count,add=1;
...
Ocorrencia *InsereOcorrencia(Ocorrencia *ocorr, int arq, char palavra, int l)
{
  Ocorrencia *p;
  ...
  p->linhas = realocaLinOcorr(p->linhas, &p->qtdOcorrencias, arq, palavra, l); //ERRO AQUI
  p->prox=NULL;
...
Palavra *InserePalavra(Palavra *structPalavra, char* palavra, int l){
    Palavra *p;
    ...
    if(ExistePalavra(structPalavra,palavra)==0){//Verificando se a palavra já existe
        ...
        structPalavra->ocorrencias=InsereOcorrencia(structPalavra->ocorrencias, 1, palavra, l); //O erro passsa por aqui
...
void LerPalavrasArquivo(Palavra *palavra, char *NameArq){//Precisa fazer adicionar em ordem alfabética
    ...
    if(arq!=NULL){
        char word[1000];
        while(fread(word,sizeof(char)*1,1,arq)==1){ //Lê uma linha do arquivo
            //printf("\n\nPalavra : %s\n\n", word);
            InserePalavra(palavra,word, 0); // O erro vem daqui
        }
    }
}
Palavra *ListaPalavrasArquivo(Palavra *palavra, char *NameArq){//Precisa fazer adicionar em ordem alfabética
    int linha=0;
    FILE *arq=fopen(NameArq,"rt");
    if(arq!=NULL){
        char word[1000];
        while(fread(word,sizeof(char),1,arq)==1){ //Lê uma linha do arquivo
            linha+=1;
            palavra=InserePalavra(palavra, word, linha); // O erro vem daqui
        }
    }
    return palavra;
}
...
void SalvaIndice(Indice *ind){
    ...
    while(arq!=NULL){//Percorrendo todos os arquivos do indice
        //fprintf(arqvotos, "%d\n", rand()%100);
        strlen(arq->nomeArquivo);
        
        fprintf(indice,"%ld", strlen(arq->nomeArquivo)+1);//Imprimindo no arquivo o tamanho do nome do arquivo
